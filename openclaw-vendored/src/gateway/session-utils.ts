...
// Disk + configured agent ID resolution lives in `./agent-list.js` so the
// listing surface, session routing, and `agents.files.set` resolver share a
// single source of truth. Removing the previous local copies here is what
// closes the registry-drift bug that produced "unknown agent id" failures
// against agents that `agents.list` had already reported as visible.
...
export function listAgentsForGateway(cfg: OpenClawConfig): {
  ...
  // Use the shared helper so every consumer of the gateway agent registry
  // (listing, session routing, file-write resolution) ends up with the same
  // ID set.
