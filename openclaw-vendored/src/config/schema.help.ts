...
export const FIELD_HELP: Record<string, string> = {
  ...
  "gateway.controlUi.allowSharedTokenBackendOperator":
    "Trusts a remote backend gateway-client that authenticates with the shared gateway token to act as a full operator without device pairing. Designed for hosted multi-tenant control planes where one trusted backend provisions tenant runtimes over the network. Only enable when the shared token is held exclusively by your control plane.",
  "gateway.push":
