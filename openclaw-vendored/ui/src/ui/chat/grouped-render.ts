    ...
    for (const block of content) {
      ...
      if (b.type === "image") {
        ...
        if (source?.type === "base64" && typeof source.data === "string") {
          ...
          // Canonical agent-tool image block: { type: "image", data, mimeType }
          // (matches the `imageResult` helper in src/agents/tools/common.ts).
          const mediaType =
            (typeof b.mimeType === "string" && b.mimeType) ||
            (typeof b.media_type === "string" && b.media_type) ||
            undefined;
          appendImageBlock(images, {
            url: buildBase64ImageUrl({ data: b.data, mediaType }),
          });
