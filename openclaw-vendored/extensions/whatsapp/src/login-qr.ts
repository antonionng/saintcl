...
export async function startWebLoginWithQr(
  ...
  // Mirrors zalouser: persist the QR PNG to a temp file so the agent tool
  // result can reference it without inlining a giant base64 data URL into
  // chat history (which gets truncated by the chat history sanitizer).
  try {
    const qrPath = await writeQrDataUrlToTempFile(login.qrDataUrl, account.accountId);
    if (qrPath) {
      login.qrPath = qrPath;
    }
  } catch (err) {
    runtime.log(info(`WhatsApp QR temp file write failed: ${String(err)}`));
  }
  return {
