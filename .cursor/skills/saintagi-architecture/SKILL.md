---
name: saintagi-architecture
description: Understand the SaintAGI control plane, including the Next.js app, Supabase org-scoped data layer, tenant OpenClaw runtime management, and the vendored OpenClaw boundary. Use when planning features, tracing ownership, or deciding which layer should change.
---

# SaintAGI Architecture
...
3. Prefer SaintAGI wrappers over direct `openclaw-vendored` edits.
...
- `openclaw-vendored/`: upstream snapshot plus SaintAGI runtime patches.
...
- Upstream-gateway behavior belongs in `openclaw-vendored` only if SaintAGI cannot solve it via configuration, wrappers, or proxying.
...
- Avoid coupling SaintAGI product logic to vendor internals unless the change is clearly runtime-native.
